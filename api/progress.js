...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const quizzes = await sql`SELECT * FROM quiz_results WHERE user_id = ${token.id} ORDER BY completed_at DESC`;
    const logs = await sql`SELECT * FROM study_logs WHERE user_id = ${token.id} ORDER BY created_at DESC LIMIT 20`;
  ...
  if (req.method === 'POST') {
    ...
    if (type === 'lesson') {
      await sql`
        ...
        VALUES (${token.id}, ${data.lessonId}, ${data.section}, true, ${data.score || null})
    ...
    if (type === 'quiz') {
      await sql`
        ...
        VALUES (${token.id}, ${data.lessonId}, ${data.score}, ${data.total}, ${JSON.stringify(data.answers || {})})
      `;
    }
    ...
    if (type === 'study_log') {
      await sql`
        ...
        VALUES (${token.id}, ${data.activityType}, ${data.duration || null}, ${JSON.stringify(data.details || {})})
      `;
    }
